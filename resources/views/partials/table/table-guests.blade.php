                    ...
                    <tr class="border-gray-100 border-y dark:border-gray-800">
                        <th class="py-3 pl-4">
                            <div class="flex items-center">
                                <button @click="sortBy('id')" class="flex items-center gap-1 font-medium text-gray-500 text-sm dark:text-gray-400 hover:text-gray-700">
                                    ID
                                    <template x-if="sort.field === 'id'">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    <template x-for="guest in guests" :key="guest.id">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            <td class="py-3 pl-4">
                                <div class="flex items-center">
                                    <p class="font-medium text-gray-800 text-sm dark:text-white/90">
                                        G<span x-text="guest.id.toString().padStart(5, '0')"></span>
                                    </p>
                                </div>
                            </td>
                            <td class="py-3">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                                        ...
                                        <template x-if="!guest.user?.avatar">
                                            <span class="text-primary font-bold text-sm" x-text="getInitials(guest.user?.name)"></span>
                                        </template>
                                    </div>
                                    <div>
                                        <a :href="`/guests/${guest.id}`"
                                           class="font-medium text-gray-800 text-sm dark:text-white/90 hover:underline"
                            ...
                            <td class="py-3">
                                <div class="flex flex-col">
                                    <span class="text-gray-800 text-sm dark:text-white/90">
                                        <span x-text="getReadableIdType(guest.id_type)"></span>: <span x-text="guest.id_number"></span>
                                    </span>
                                </div>
                            </td>
                            <td class="py-3">
                            ...
                            <td class="py-3">
                                <div class="flex flex-col">
                                    <template x-if="guest.emergency_contact?.name">
                                        <span class="text-gray-800 text-sm dark:text-white/90" x-text="guest.emergency_contact.name"></span>
                                        <span class="text-gray-500 text-xs dark:text-gray-400" x-text="guest.emergency_contact.relationship || 'N/A'"></span>
                                    </template>
                                    <template x-if="!guest.emergency_contact?.name">
                                        <span class="text-gray-500 text-sm dark:text-gray-400">N/A</span>
                                    </template>
                                </div>
                            </td>
                            <td class="py-3">
                            ...
                            <td class="py-3">
                                <div class="flex items-center">
                                    <template x-if="guest.user?.currentStay?.roomAllocations?.[0]?.room">
                                        <p class="text-gray-800 text-sm dark:text-white/90">
                                            <span x-text="guest.user.currentStay.roomAllocations[0].room.room_number"></span>
                                        </p>
                                    </template>
                                    <template x-if="!guest.user?.currentStay?.roomAllocations?.[0]?.room">
                            ...
                            <td class="py-3 pr-4">
                                <div x-data="dropdown()" class="relative">
                                    ...
                                    <div x-show="open" @click.outside="open = false" class="shadow-theme-lg dark:bg-gray-dark fixed w-40 space-y-1 rounded-2xl border border-gray-200 bg-white p-2 dark:border-gray-800" x-ref="dropdown">
                                        <a :href="`/guests/${guest.id}`" 
                                        ...
                                        <button @click="confirmDeleteGuest(guest.id, guest.user?.name)"
                                                class="text-theme-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-red-600 hover:bg-gray-100 hover:text-red-700 dark:text-red-400 dark:hover:bg-white/5 dark:hover:text-red-300">
                                            Delete
                                        </button>
                                    </div>
                                </div>
            ...
            <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between">
                ...
                <div class="flex items-center justify-center gap-0.5 pt-4 xl:justify-end xl:pt-0">
                    ...
                    <template x-for="page in getVisiblePages()" :key="page">
                        <button @click="goToPage(page)" :class="pagination.currentPage === page ? 'bg-blue-500/[0.08] text-brand-500' : 'text-gray-700 dark:text-gray-400'"
...
<div x-show="showGuestModal" x-cloak class="fixed inset-0 z-99999 overflow-y-auto">
    <div x-show="showGuestModal" 
        ...
        <div class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]" @click="showGuestModal = false"></div>
        ...
        <div x-show="showGuestModal"
             ...
             @click.outside="showGuestModal = false"
             class="relative w-full max-w-[800px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
            ...
            <div class="max-h-[80vh] overflow-y-auto">
                <h4 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90" x-text="editingGuest ? 'Edit Guest' : 'Add New Guest'"></h4>
                
                <form id="guestForm" @submit.prevent="editingGuest ? updateGuest() : createGuest()" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Personal Information -->
                        <div>
                            <h4 class="text-sm font-semibold text-gray-700 mb-4 pb-2 border-b dark:text-gray-300">Personal Information</h4>
                            <div class="space-y-4">
                                <div>
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-gray-700 mb-4 pb-2 border-b dark:text-gray-300">Identification & Preferences</h4>
                            <div class="space-y-4">
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                        ID Type *
                    ...
                    <div class="border-t border-gray-100 pt-6 dark:border-gray-800">
                        <h4 class="mb-6 text-sm font-semibold text-gray-700 dark:text-gray-300">Guest Preferences</h4>
                        ...
                        <div x-data="{
                            ...
                            <div @click="open = !open"
                                class="dark:bg-dark-900 flex min-h-11 cursor-pointer gap-2 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 shadow-theme-xs transition dark:border-gray-700 dark:bg-gray-900">
                                <div class="flex flex-1 flex-wrap items-center gap-2">
                                    ...
                                    <template x-if="selected.length === 0">
                                        <span class="text-gray-400 dark:text-white/30">Select room preferences...</span>
                                    </template>
                                </div>
                                ...
                                <div class="max-h-60 overflow-y-auto p-2">
                                    <template x-for="option in options" :key="option.id">
                                        <div @click="toggleOption(option.id)"
                                            :class="isSelected(option.id) ? 'bg-brand-500/10 text-brand-600 dark:text-brand-400' : 'hover:bg-gray-50 dark:hover:bg-gray-800'"
                                            class="flex cursor-pointer items-center rounded-lg px-3 py-2 text-sm dark:text-white/90">
                                            <div class="flex h-5 w-5 items-center justify-center rounded border border-gray-300 mr-2 dark:border-gray-600">
                                                <template x-if="isSelected(option.id)">
                                                    <svg class="fill-current text-brand-600 dark:text-brand-400" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div x-data="{
                            ...
                            <div @click="open = !open"
                                ...
                                <div class="flex flex-1 flex-wrap items-center gap-2">
                                    ...
                                    <template x-if="selected.length === 0">
                                        <span class="text-gray-400 dark:text-white/30">Select entertainment preferences...</span>
                                    </template>
                                </div>
                                <span class="flex items-center text-gray-400">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div x-data="{
                            ...
                            <div @click="open = !open"
                                ...
                                <div class="flex flex-1 flex-wrap items-center gap-2">
                                    ...
                                    <template x-if="selected.length === 0">
                                        <span class="text-gray-400 dark:text-white/30">Select room service preferences...</span>
                                    </template>
                                </div>
                                <span class="flex items-center text-gray-400">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div x-data="{
                            ...
                            <div @click="open = !open"
                                ...
                                <div class="flex flex-1 flex-wrap items-center gap-2">
                                    ...
                                    <template x-if="selected.length === 0">
                                        <span class="text-gray-400 dark:text-white/30">Select restaurant preferences...</span>
                                    </template>
                                </div>
                                <span class="flex items-center text-gray-400">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div x-data="{
                            ...
                            <div @click="open = !open"
                                ...
                                <div class="flex flex-1 flex-wrap items-center gap-2">
                                    ...
                                    <template x-if="selected.length === 0">
                                        <span class="text-gray-400 dark:text-white/30">Select check-in time preferences...</span>
                                    </template>
                                </div>
                                <span class="flex items-center text-gray-400">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <!-- Emergency Contact -->
                    <div class="border-t border-gray-100 pt-6 dark:border-gray-800">
                        <h4 class="mb-4 text-sm font-semibold text-gray-700 dark:text-gray-300">Emergency Contact</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    ...
                    <div class="flex items-center justify-end w-full gap-3 mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
                        ...
                        >
                            ...
                            <span x-show="isSubmitting" class="flex items-center">
                                ...
                                <span x-text="editingGuest ? 'Updating...' : 'Creating...'"></span>
                            </span>
                        </button>
                    </div>
                </form>
            ...
            <div x-show="showRedirectModal"
                ...
                <div class="mt-6 space-y-3">
                    <button @click="redirectToGuest()" 
                            class="flex w-full items-center justify-center rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition-colors hover:bg-brand-600">
                    ...
                    <button @click="redirectToGuestWithStay()" 
                            class="flex w-full items-center justify-center rounded-lg bg-green-600 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition-colors hover:bg-green-700">
                        <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
openEditModal(guest) {
    ...
    if (guest.preferences) {
        ...
        showToast(message, type = 'info') {
            ...
            toast.className = `custom-toast fixed top-4 right-4 px-4 py-3 rounded-lg text-white text-sm shadow-lg z-50 transition-transform duration-300 transform ${
